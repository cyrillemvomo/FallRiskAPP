...
def nameRoute():
    ...
    if(request.method == 'POST'):
        ...
        score = (1-Loaded_Model.predict_proba(np.array(data).reshape(1, -1))[0][1]) * 100
        label_class= Loaded_Model.predict(np.array(data).reshape(1, -1))[0]
        if label_class > 0:
            label = 'HIGH'
        else:
            label = "LOW"
        response = f'Balance Score is : {round(score, 2)}/100 and Fall Risk is : {label}' #re-assigning response with the name we got from the user
